...
// Impor router dan todos dari file todo.js yang sudah diperbarui
const { router: todoRouter, todos } = require("./routes/todo.js");
const todoRoutes = require("./routes/tododb.js");
const db = require("./Database/db"); // Impor koneksi database
const expressLayouts = require("express-ejs-layouts");
app.use(expressLayouts);

app.use(express.json());
// Pastikan baris ini ada untuk memproses data dari form
app.use(express.urlencoded({ extended: true }));

// Gunakan router dengan prefix /todos
app.use("/todos", todoRoutes);

// Atur EJS view engine
...
app.get("/todos-data", (req, res) => {
  res.json(todos); // Endpoint ini tetap berfungsi untuk API
});

// Halaman ini sekarang akan selalu menampilkan data 'todos' yang terbaru
app.get("/todos-list", (req, res) => {
  res.render("todos-pages", { layout: "layouts/main-layout", todos: todos });
});

app.get("/todo-view", (req, res) => {
  db.query('SELECT * FROM todos', (err, todos) => {
    if (err) return res.status(500).send('Internal Server Error');
    res.render("todo", { layout: "layouts/main-layout", todos: todos });
  });
});
